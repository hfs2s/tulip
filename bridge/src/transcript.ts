...
/**
 * An attachment on a message, as the Chat page draws it.
 *
 * Deliberately not the file name. The panel plays a recording by naming the
 * *row* it hangs off — `SaidItem.uid` — and the bridge looks the file up from
 * its own feed, so the browser never holds a path and never sends one. What it
 * does hold is enough to label the thing before it is played: which kind, how
 * long, whether it was spoken or attached, and the words, if any were made out.
 */
export interface SaidMedia {
  readonly kind: string;
  readonly bytes: number | null;
  readonly seconds: number | null;
  /** A held-to-record voice note rather than an audio file. */
  readonly voice: boolean;
  /** What was said in it, from the transcriber's sidecar. Null when there is none. */
  readonly transcript: string | null;
  /** Whether the bridge still has the file, so the panel offers no player for a gone one. */
...
export interface SaidItem {
  ...
  readonly text: string;
  /**
   * The feed row this came from, on inbound messages that carry an attachment.
   *
   * It is the handle `/api/chat/media` takes: a row id, generated by the bridge
   * and unique per line of the feed, which the bridge resolves back to a file
   * on its own side. Absent on rows with nothing to fetch, so the panel has
   * nothing to build a URL from where there is nothing to serve.
   */
  readonly uid?: string;
  /** Attachments on this message, oldest first. Absent when there are none. */
  readonly media?: readonly SaidMedia[];
  /**
