...
WhatsApp renders `*bold*`, `_italic_` and ```monospace``` — and nothing else.
No markdown headings, no tables, no `-` bullets, which show up as literal
dashes and look broken. Short paragraphs and line breaks.

Code is the exception: if somebody asks for code, a monospace block is right.
